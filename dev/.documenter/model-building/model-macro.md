...
## Default Behavior for Omitted Blocks {#Default-Behavior-for-Omitted-Blocks}
...
## Example: Nonlinear Mixed-Effects Model (No ODE) {#Example:-Nonlinear-Mixed-Effects-Model-No-ODE}
...
## Example: Full ODE Model {#Example:-Full-ODE-Model}
...
`@Model` has a second form that builds a new model from an existing one, overriding only the blocks you supply:
...
Every model built by `@Model` stores the source expression of each of its blocks (see [`get_source`](/api#NoLimits.get_source)). The extension form reuses those stored blocks for anything you do not mention, so you only restate what actually changes between the two models.

### Merge semantics {#Merge-semantics}

Within an overridden block, entries are merged **by name** – the left-hand side of each declaration:
- An entry whose name matches one already in the base block **replaces** it, keeping its original position.
...
A block you do not mention at all is inherited verbatim. The merged result is rebuilt through the full single-argument `@Model` pipeline, so **all normal validation applies** (required `@formulas`, at least one parameter, DE/initial-DE pairing, covariate-usage rules, etc.).

### Example: Gaussian → normalizing-flow random effect {#Example:-Gaussian-normalizing-flow-random-effect}
...
the flow variant changes only three blocks – `@covariates`, `@fixedEffects` keeps all parameters except `sigma_k` (removed) and adds `psi`, `eta_k` becomes a flow while `eta_L` is inherited, and the `length` formula switches `eta_k` to `eta_k[1]`:
...
`model_flow` ends up with fixed effects `L_inf_pop, k_pop, sigma_L, sigma_y, psi` and random effects `eta_k` (flow) and `eta_L` (inherited Gaussian). Extension models can themselves be extended – the new model again stores its own source.

::: warning Blocks must be self-contained

Inherited and overriding blocks are re-evaluated in the module where the extension call occurs. Any variable they reference (for example a `Lux.Chain`, a `knots` vector, or a precomputed matrix) must be available as a global in that module. Local variables captured at the base model's original call site are **not** carried over – declare such values as globals, or re-declare them before the extension call.
...
## Runtime Evaluation Helpers {#Runtime-Evaluation-Helpers}
