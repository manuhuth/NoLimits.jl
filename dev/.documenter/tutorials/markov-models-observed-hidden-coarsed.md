...
# Markov Models in NoLimits.jl: Observed, Hidden, Coarsed, and Pathsum {#Markov-Models-in-NoLimits.jl:-Observed,-Hidden,-Coarsed,-and-Pathsum}
...
## Step 0: Setup and Data generation {#Step-0:-Setup-and-Data-generation}
...
### Define Relevant Model Components {#Define-Relevant-Model-Components}
...
## Step 1: Fit a Discrete-Time MM with observed state space {#Step-1:-Fit-a-Discrete-Time-MM-with-observed-state-space}
...
### Parameterization notes {#Parameterization-notes}
...
## Step 2: Fit a Continuous-Time HMM in NoLimits {#Step-2:-Fit-a-Continuous-Time-HMM-in-NoLimits}

The previous model assumed that the Markov state is directly observed. In many applications this is unrealistic: the true state is latent and one observes noisy proxies generated from that state.
...
### Parameterization notes {#Parameterization-notes-2}
...
## Step 3: Coarsed Observations {#Step-3:-Coarsed-Observations}

In some studies, the observed state is ambiguous. Instead of observing a single label, one may only know that the process is in one of several possible states, e.g. `{2,3}`.
...
## Step 4: Optional Pathsum for an Acyclic CT Model {#Step-4:-Optional-Pathsum-for-an-Acyclic-CT-Model}
...
## Step 5: Random Effects and Other Flexible Parameterizations {#Step-5:-Random-Effects-and-Other-Flexible-Parameterizations}
