...
If the model has no random effects, `Laplace` will raise an error. Use a fixed-effects method such as [`MLE`](mle.md) or [`MAP`](/estimation/mle#MAP-Estimation) instead.
...
See the [`Laplace`](/estimation/laplace#Laplace) entry in the API reference for the full list of keyword arguments and their defaults.
...
### Inner vs Outer Optimizer Choices (Optimization.jl Interface) {#Inner-vs-Outer-Optimizer-Choices-Optimization.jl-Interface}
...
This section explains the behaviour of the options whose effect is not obvious from their name. See the [`Laplace`](/estimation/laplace#Laplace) entry in the API reference for the complete keyword list and defaults.
...
### Advanced Option Containers {#Advanced-Option-Containers}
...
## Fixing Known Random-Effect Levels (`constants_re`) {#Fixing-Known-Random-Effect-Levels-constants_re}
...
## Bounds and BlackBoxOptim {#Bounds-and-BlackBoxOptim}
