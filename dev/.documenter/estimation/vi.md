...
::: tip VI is fixed-effects only

VI is not supported for models with random effects. For full Bayesian inference on mixed-effects models use [`MCMC`](mcmc.md) instead. For a worked fixed-effects VI example, see the [Fixed-Effects VI tutorial](../tutorials/fixed-effects-vi.md).
...
## Basic Usage (Fixed Effects) {#Basic-Usage-Fixed-Effects}
...
## Accessing VI Outputs {#Accessing-VI-Outputs}
...
## Uncertainty Quantification with VI {#Uncertainty-Quantification-with-VI}
...
## Multistart with VI {#Multistart-with-VI}
