...
## Constructor Input Reference {#Constructor-Input-Reference}

The constructor signature block above lists every keyword with its default. See the [`SAEM`](/estimation/saem#SAEM) entry in the API reference for the full list. This section explains the behavior of the option groups whose effect is not obvious from their names.

### E-step Sampling Inputs {#E-step-Sampling-Inputs}
...
- `sampler`
  ...
  - Default: `SaemixMH()`. See [Samplers](/estimation/saem#samplers) for all available options.
...
### M-step Optimization Inputs {#M-step-Optimization-Inputs}
...
### E-step Retry Inputs {#E-step-Retry-Inputs}
...
### SA Schedule Inputs {#SA-Schedule-Inputs}
...
#### `:robbins_monro` schedule {#:robbins_monro-schedule}
...
#### `:two_phase` schedule {#:two_phase-schedule}
...
### Multi-Chain E-step {#Multi-Chain-E-step}
...
### SA Variance Annealing {#SA-Variance-Annealing}
...
### Variance Lower Bound {#Variance-Lower-Bound}
...
### Custom Statistics Inputs {#Custom-Statistics-Inputs}
...
### Adaptive Q Memory Policy Inputs {#Adaptive-Q-Memory-Policy-Inputs}
...
- `q_store_epsilon` (default `1e-10`)
  - Weight pruning threshold. After each push, snapshots whose SA weight falls below this value are removed from the oldest end of the buffer (subject to `q_store_min`). During the γ=1 stabilisation phase all previous snapshots are immediately pruned, keeping only the current iteration's sample in the buffer.
    
  - The retained weights are renormalised to sum to 1 before evaluating Q, so the objective is scale-invariant to pruning.
    
  - Has no effect when `suffstats` is provided; a warning is emitted if set to a non-default value alongside `suffstats`.
    
  
- `q_store_min` (default `0`)
  - Guaranteed minimum number of retained snapshots. When epsilon pruning would reduce the active count below this floor, the most-recent snapshots are kept unconditionally regardless of their weight.
    
  

### Built-in Update Inputs {#Built-in-Update-Inputs}
...
### Final EB Mode Inputs {#Final-EB-Mode-Inputs}

After convergence, SAEM computes empirical Bayes modal estimates of the random effects for use by downstream accessors and diagnostics, configured through the `ebe_*` keywords. When `ebe_rescue_on_high_grad=true` (default `false`), a rescue strategy governed by the `ebe_rescue_*` keywords is activated if the final EB gradient norm remains above threshold. See the [`SAEM`](/estimation/saem#SAEM) API entry for the full keyword list and defaults.
...
### RE Annealing Inputs {#RE-Annealing-Inputs}
- `anneal_to_fixed`
...
### `SaemixMH` (default) {#SaemixMH-default}
...
### `AdaptiveNoLimitsMH` {#AdaptiveNoLimitsMH}
...
### Turing-Based Samplers (`NUTS`, etc.) {#Turing-Based-Samplers-NUTS,-etc.}
...
### Interaction with Built-in Statistics {#Interaction-with-Built-in-Statistics}
...
## Which Models Have Closed-Form M-step Updates? {#Which-Models-Have-Closed-Form-M-step-Updates?}
...
### Example 1: Neural-Network-Based Nonlinear ODE Model with Closed-Form RE-Mean and Outcome-Scale Blocks {#Example-1:-Neural-Network-Based-Nonlinear-ODE-Model-with-Closed-Form-RE-Mean-and-Outcome-Scale-Blocks}
...
### Example 2: Soft-Decision-Tree-Based Nonlinear ODE Model with Closed-Form RE-Mean and Outcome-Scale Blocks {#Example-2:-Soft-Decision-Tree-Based-Nonlinear-ODE-Model-with-Closed-Form-RE-Mean-and-Outcome-Scale-Blocks}
...
### Example 3: Mechanistic ODE with Auto-Detected Closed-Form Blocks {#Example-3:-Mechanistic-ODE-with-Auto-Detected-Closed-Form-Blocks}
...
## Custom Sufficient Statistics and Closed-Form M-step {#Custom-Sufficient-Statistics-and-Closed-Form-M-step}
...
## Optimization.jl Interface Example {#Optimization.jl-Interface-Example}
