...
## E-Step Strategies {#E-Step-Strategies}

MCEM supports two E-step implementations, controlled by the `e_step` argument.

### MCMC E-step (`MCEM_MCMC`) {#MCMC-E-step-MCEM_MCMC}
...
### Importance Sampling E-step (`MCEM_IS`) {#Importance-Sampling-E-step-MCEM_IS}
...
- `proposal` — proposal distribution (see [Proposal Modes](/estimation/mcem#Proposal-Modes) below).
...
#### MCMC Warm-up then IS {#MCMC-Warm-up-then-IS}
...
#### User-Supplied Proposal {#User-Supplied-Proposal}
...
The constructor signature block above lists every keyword with its default. See the [`MCEM`](/estimation/mcem#MCEM) entry in the API reference for the full list. The notes below explain the behavior that is not obvious from the names.
...
## Optimization.jl Interface (M-step and EB) {#Optimization.jl-Interface-M-step-and-EB}
