...
function _simulate_fit(model, n_ids, n_obs; rng::AbstractRNG)
    ...
    res = fit_model(dm_sim, MLE(); theta_0_untransformed=θ_init, rng=rng)
    return res, θ_init
end

function run_mle_validation(; n_reps=400, n_ids=100, n_obs=5, rel_tol=0.1, abs_tol=0.1, seed=1)
    ...
    for (mi, model) in enumerate(models)
        ...
        p = use_progress ? Progress(n_reps, desc="MLE validation model $(mi)") : nothing
        if use_progress
            for r in 1:n_reps
        ...
        @info "MLE validation start summary model $(mi)" n_reps n_ids n_obs rel_tol abs_tol start_stats
    end

    @info "MLE validation summary" n_reps n_ids n_obs rel_tol abs_tol stats
    return stats
end

stats = run_mle_validation(; n_reps=400, n_ids=500, n_obs=10, rel_tol=0.05, abs_tol=0.1, seed=1);
