...
function _simulate_fit(model, n_ids, n_obs; rng::AbstractRNG)
    ...
    println("Fitting Laplace with initial parameters: ", θ_init)
    res = fit_model(dm_sim, Laplace( ); theta_0_untransformed=θ_init, rng=rng)
    return res, θ_init
end

function run_laplace_validation(; n_reps=400, n_ids=100, n_obs=5, rel_tol=0.1, abs_tol=0.1, seed=1)
    model = _build_model()
    ...
    p = use_progress ? Progress(n_reps, desc="Laplace validation") : nothing
    ...
    @info "Laplace validation summary" n_reps n_ids n_obs rel_tol abs_tol stats start_stats n_valid n_failed=failed[]
    return stats
end

stats = run_laplace_validation(; n_reps=1, n_ids=100, n_obs=5, rel_tol=0.1, abs_tol=0.1, seed=1);
