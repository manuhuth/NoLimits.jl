...
function _simulate_fit(model, n_ids, n_obs; rng::AbstractRNG)
    ...
    res = fit_model(dm_sim,
                     SAEM(; sampler=MH(),
                            turing_kwargs=(n_samples=30, n_adapt=0, progress=false, verbose=false),
                            t0=10, kappa=0.7,
                            max_store=20, verbose=false,
                            consecutive_params=10, maxiters=50,),
                    theta_0_untransformed=θ_init,
                    rng=rng)
    return res, θ_init
end

function run_saem_validation(; n_reps=400, n_ids=100, n_obs=5, rel_tol=0.1, abs_tol=0.1, seed=1)
    model = _build_model()
    ...
    p = use_progress ? Progress(n_reps, desc="SAEM validation") : nothing
    ...
    @info "SAEM validation summary" n_reps n_ids n_obs rel_tol abs_tol stats start_stats n_valid n_failed=failed[]
    return stats
end

stats = run_saem_validation(; n_reps=10, n_ids=100, n_obs=5, rel_tol=0.1, abs_tol=0.1, seed=1);
