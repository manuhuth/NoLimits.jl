...
function _simulate_fit(model, n_ids, n_obs; rng::AbstractRNG)
    ...
    res = fit_model(dm_sim,
                    MCEM(; sampler=MH(), turing_kwargs=(n_samples=40, n_adapt=0, progress=false), optim_kwargs=(;),
                          maxiters=30,
                        rtol_theta=1e-3,
                        atol_theta=1e-5,
                        rtol_Q=1e-3,
...
function run_mcem_validation(; n_reps=400, n_ids=100, n_obs=5, rel_tol=0.1, abs_tol=0.1, seed=1)
    model = _build_model()
    ...
    p = use_progress ? Progress(n_reps, desc="MCEM validation") : nothing
    ...
    @info "MCEM validation summary" n_reps n_ids n_obs rel_tol abs_tol stats start_stats n_valid n_failed=failed[]
    return stats
end

stats = run_mcem_validation(; n_reps=10, n_ids=100, n_obs=5, rel_tol=0.1, abs_tol=0.1, seed=1);
