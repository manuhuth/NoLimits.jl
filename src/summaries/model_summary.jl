...
function summarize(m::Model)
        ...
        push!(fixed_effect_summaries,
            (;
                ...
                calculate_se = hasproperty(p, :calculate_se) ? getfield(p, :calculate_se) :
                               false,
                prior_type = _ms_prior_type(hasproperty(p, :prior) ? getfield(p, :prior) :
                                            Priorless()),
    ...
    for re in re_names
        push!(random_effect_summaries,
            (; name = re, group = getfield(re_groups, re),
...
function Base.show(io::IO, ::MIME"text/plain", s::ModelSummary)
    ...
    if isempty(s.fixed_effect_summaries)
        ...
        name_w = max(length("name"),
            maximum(length(string(r.name)) for r in s.fixed_effect_summaries))
        type_w = max(length("type"),
            maximum(length(string(r.block_type)) for r in s.fixed_effect_summaries))
        size_w = max(length("size"),
            maximum(length(string(r.size)) for r in s.fixed_effect_summaries))
        se_w = length("se")
        prior_w = max(length("prior"),
            maximum(length(string(r.prior_type)) for r in s.fixed_effect_summaries))
        scale_w = max(length("scale"),
            maximum(length(string(r.scale)) for r in s.fixed_effect_summaries))
        bounds_w = max(length("bounds"),
            maximum(length(string(r.bounds)) for r in s.fixed_effect_summaries))
        println(io, "  ",
            rpad("name", name_w), "  ",
            rpad("type", type_w), "  ",
            lpad("size", size_w), "  ",
        ...
        println(io,
            "  ",
            repeat(
                "-", name_w + type_w + size_w + se_w + prior_w + scale_w + bounds_w + 34))
    ...
    if isempty(s.covariate_summaries)
        ...
        name_w = max(
            length("name"), maximum(length(string(r.name)) for r in s.covariate_summaries))
        kind_w = max(
            length("kind"), maximum(length(string(r.kind)) for r in s.covariate_summaries))
        println(io, "  ",
            rpad("name", name_w), "  ",
            rpad("kind", kind_w), "  ",
            rpad("columns", 24), "  ",
