...
function _mcmc_fixed_means(res::FitResult,
    ...
    return ComponentArray(NamedTuple(pairs)), chain
end
...
function build_plot_cache(res::FitResult;
    ...
    θ_chain = res.result isa MCMCResult ? _mcmc_fixed_means(res, dm; max_draws=mcmc_draws, rng=rng) : nothing
    θ = res.result isa MCMCResult ? θ_chain[1] : get_params(res; scale=:untransformed)
    θ = _apply_param_overrides(θ, params)
    chain = res.result isa MCMCResult ? θ_chain[2] : nothing
