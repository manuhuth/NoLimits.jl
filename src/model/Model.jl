...
function Model(fixed, random, covariates, de, helpers, formulas)
    return Model(fixed, random, covariates, de, helpers, formulas, nothing)
end
...
function _initial_state_with_pre(
        m::Model,
        θ::ComponentArray,
        η::ComponentArray,
        const_covariates_i::NamedTuple,
...
"""
function calculate_formulas_all(
        m::Model,
        θ::ComponentArray,
        η::ComponentArray,
        const_covariates_i::NamedTuple,
    ...
    pre = calculate_prede(m, θ, η, const_covariates_i)
    ctx = (;
        fixed_effects = θ, random_effects = η, prede = pre,
...
"""
function calculate_formulas_obs(
        m::Model,
        θ::ComponentArray,
        η::ComponentArray,
        const_covariates_i::NamedTuple,
...
macro Model(block)
    ...
    return quote
        ...
        local $(de_bundle_var) = DEBundle(
            $(de_var), $(initial_var), $(prede_var),
        ...
        local (
            $(form_all_var), $(form_obs_var), $(req_states_var),
            $(req_signals_var),
...
function _is_removal(ln)
    return ln isa Expr && ln.head == :(=) && length(ln.args) == 2 &&
...
function _wrap_block(head::Symbol, body::Expr)
    return Expr(:macrocall, Symbol("@", head), LineNumberNode(0, Symbol("@", head)), body)
end
