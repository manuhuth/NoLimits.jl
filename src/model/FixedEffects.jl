...
function _collect_model_fun!(p::NNParameters, model_fun_pairs, ::Type{T}) where {T}
    push!(model_fun_pairs, p.function_name => _nn_model_fun(p.chain, p, T))
    return nothing
end

# Backend-specific forward pass. Methods live in NoLimitsLuxExt and
# NoLimitsSimpleChainsExt; dispatch is on the chain object rather than on NNParameters'
# chain type parameter, so neither extension has to name the other's types (and so the
# free `Type{T}` cannot interfere with specificity).
function _nn_model_fun(chain, p::NNParameters, ::Type{T}) where {T}
    return error("NN parameter $(p.name): no forward pass is defined for a chain of type $(typeof(chain)). A `Lux.Chain` needs `using Lux` and a `SimpleChains.SimpleChain` needs `using SimpleChains`; both are optional dependencies of NoLimits.")
end
