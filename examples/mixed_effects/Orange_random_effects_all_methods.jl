...
if quick_mode
    println("Running Orange example in quick mode (LD_EXAMPLE_QUICK=true).")
else
    println("Running Orange example in full mode.")
end
...
model = @Model begin
    ...
    @fixedEffects begin
        ...
        phi2 = RealNumber(190.0, prior=LogNormal(log(190.0), 0.5), calculate_se=true)
        phi3 = RealNumber(700.0, prior=LogNormal(log(700.0), 0.5), calculate_se=true)
        omega = RealNumber(1.0, scale=:log, prior=LogNormal(log(15.0), 0.5), calculate_se=true)
        sigma = RealNumber(1.0, scale=:log, prior=LogNormal(log(6.0), 0.5), calculate_se=true)
    end
    ...
    @formulas begin
        max_growth_i = phi2 + eta
        mu = phi1 + (max_growth_i - phi1) / (1 + exp(-(age - phi3) / 100))
...
laplace_method = quick_mode ?
    ...
    NoLimits.Laplace(; multistart_n=20, multistart_k=5)
...
focei_method = quick_mode ?
    ...
    NoLimits.FOCEI(; info_mode=:fisher_common, multistart_n=20, multistart_k=5)
...
mcmc_turing_kwargs = quick_mode ?
    ...
    (n_samples=2500, n_adapt=1000, progress=true)
mcmc_method = NoLimits.MCMC(; turing_kwargs=mcmc_turing_kwargs)

uq_draws = quick_mode ? 300 : 3000
uq_mcmc_warmup = mcmc_turing_kwargs.n_adapt
uq_mcmc_draws = quick_mode ? 80 : 1000
plot_mcmc_draws = quick_mode ? 100 : 500
