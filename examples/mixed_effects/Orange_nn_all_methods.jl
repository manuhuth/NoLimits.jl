...
if quick_mode
    println("Running Orange NN example in quick mode (LD_EXAMPLE_QUICK=true).")
else
    println("Running Orange NN example in full mode.")
end

chain = Chain(Dense(2, 6, tanh), Dense(6, 1))
...
model = @Model begin
    ...
    @formulas begin
        x1 = age / 1600
        x2 = x1^2
        latent = theta0 + NN1([x1, x2], ζ)[1] + eta
        mu = exp(latent)
