...
if quick_mode
    println("Running ChickWeight example in quick mode (LD_EXAMPLE_QUICK=true).")
else
    println("Running ChickWeight example in full mode.")
end
...
model_chick = @Model begin
    ...
    @fixedEffects begin
        ...
        #omega_eta = RealNumber(5.0, prior=LogNormal(log(10.0), 0.75), calculate_se=true, scale=:log)
        sigma = RealNumber(10.0, scale=:log, prior=LogNormal(log(10.0), 0.5), calculate_se=true)
    end

    @randomEffects begin
        #eta = RandomEffect(Normal(beta, omega_eta); column=:ID)
...
if quick_mode
    ...
    uq_refit_kwargs = (;
        mcmc_turing_kwargs=(n_samples=300, n_adapt=120, progress=false, initial_params=DynamicPPL.InitFromPrior()),
        mcmc_warmup=120,
        mcmc_draws=120
    )
    uq_chain_draws = 200
    ...
    # In full mode, use package-default estimator settings.
    laplace_method = NoLimits.Laplace(optim_kwargs=(show_trace=true,))
    focei_method = NoLimits.FOCEI(; info_mode=:fisher_common, optim_kwargs=(show_trace=true,))
    mcem_method = NoLimits.MCEM(;
        maxiters=50,
    ...
    uq_refit_kwargs = (;
        mcmc_turing_kwargs=(n_samples=1000, n_adapt=600, progress=true, initial_params=DynamicPPL.InitFromPrior()),
        mcmc_warmup=600,
        mcmc_draws=5000
    )
    uq_chain_draws = 5000
...
println("Completed ChickWeight example across Laplace, FOCEI, MCEM, SAEM, and MCMC.")
