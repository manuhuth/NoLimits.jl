...
if quick_mode
    println("Running Orange SoftTree example in quick mode (LD_EXAMPLE_QUICK=true).")
else
    println("Running Orange SoftTree example in full mode.")
end

st0 = SoftTreeParameters(2, 2; function_name=:ST_TMP, calculate_se=false);
...
model = @Model begin
    ...
    @formulas begin
        x1 = age / 1600
        x2 = x1^2
        latent = theta0 + ST1([x1, x2], Γ)[1] + eta
        mu = exp(latent)
