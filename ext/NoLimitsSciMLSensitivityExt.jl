...
function recursive_copyto!(y::AbstractArray, x::DEParams)
    return copyto!(y, vcat(collect(x.θ), collect(x.η)))
end
recursive_copyto!(y::DEParams, x::DEParams) = (
    copyto!(y.θ, x.θ);
    copyto!(y.η, x.η);
    y
)
recursive_neg!(x::DEParams) = (x.θ .*= -1; x.η .*= -1; x)
recursive_add!(y::DEParams, x::DEParams) = (
    y.θ .+= x.θ;
    y.η .+= x.η;
    y
)
recursive_sub!(y::DEParams, x::DEParams) = (
    y.θ .-= x.θ;
    y.η .-= x.η;
    y
)
