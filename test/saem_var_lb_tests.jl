...
# Helper: minimal Normal-RE DataModel on the shared fx_re_model (RE sd is ω)
...
@testset "var lb integration: Normal RE — lb prevents collapse" begin
    ...
    # ω and σ must stay ≥ 1e-5 after clamping
...
@testset "var lb integration: min rule — anneal_min_sd < var_lb_value" begin
    ...
    # anneal_to_fixed requires a literal SD in the RE distribution → fx_tiny_re_model.
    dm = fx_tiny_re_dm()
    res = fit_model(
        dm,
        NoLimits.SAEM(;
...
# One two-RE model shared by all three EBE testsets; the partial/one-RE cases
# simply leave b un-annealed.
const _VARLB_EBE_MODEL = @Model begin
    @fixedEffects begin
        mu_a = RealNumber(1.0)
        mu_b = RealNumber(0.5)
...
@testset "anneal_to_fixed EBE: scalar Normal — values equal distribution mean" begin
    ...
    dm = DataModel(_VARLB_EBE_MODEL, df; primary_id = :ID, time_col = :t)
...
@testset "anneal_to_fixed EBE: non-annealed RE still varies freely" begin
    ...
    @test :b_1 in propertynames(re.b)
    @test nrow(re.b) == 6
end
...
# ══ SAEM multi-chain tests (merged from saem_multichain_tests.jl) ═════════════
...
# 3-id df on the shared literal-SD fixture model.
function _mc_dm()
    df = DataFrame(
        ID = [:A, :A, :B, :B, :C, :C], t = [0.0, 1.0, 0.0, 1.0, 0.0, 1.0],
        y = [0.1, 0.2, 0.0, -0.1, 0.15, 0.05]
    )
    return DataModel(fx_tiny_re_model(), df; primary_id = :ID, time_col = :t)
end
...
@testset "Multi-chain: auto_small_n_chains no inflation when n_batches >= target" begin
    # Large dataset where batches ≥ small_n_chain_target:
    ...
    dm = DataModel(fx_tiny_re_model(), df; primary_id = :ID, time_col = :t)
    res = fit_model(
        dm,
        NoLimits.SAEM(;
            sampler = MH(),
