...
@testset "SA anneal: _saem_sa_anneal_floor_scalar" begin
    ...
    @test NoLimits._saem_sa_anneal_floor_scalar(1.0, 0.9, 1.0, :exponential) ≈
        0.9 * exp(-3.0)
...
@testset "SA anneal: _saem_apply_sa_anneal_clamp — scalar param" begin
    ...
    result = NoLimits._saem_apply_sa_anneal_clamp(
        θu, active, initial, 1, 10, 0.9, :exponential
    )
    ...
    result2 = NoLimits._saem_apply_sa_anneal_clamp(
        θu_large, active, initial, 1, 10, 0.9, :exponential
    )
    @test result2 === θu_large

    # After anneal period: no clamp
    result3 = NoLimits._saem_apply_sa_anneal_clamp(
        θu, active, initial, 11, 10, 0.9, :exponential
    )
    @test result3 === θu

    # Empty active targets: passthrough
    result4 = NoLimits._saem_apply_sa_anneal_clamp(
        θu, NamedTuple(), NamedTuple(), 1, 10, 0.9, :exponential
    )
    @test result4 === θu
end
...
@testset "SA anneal: _saem_apply_sa_anneal_clamp — vector param" begin
    ...
    result2 = NoLimits._saem_apply_sa_anneal_clamp(
        θu2, active, initial, 1, 10, 0.9, :exponential
    )
    @test result2 !== θu2
...
@testset "SA anneal: Normal RE — disabled when sa_anneal_alpha=0.0" begin
    ...
    res = fit_model(
        ...
        NoLimits.SAEM(;
            ...
            turing_kwargs = (n_samples = 2, n_adapt = 2, progress = false),
            maxiters = 2, t0 = 2, progress = false, q_store_max = 2, builtin_stats = :none,
            sa_anneal_iters = 4, sa_anneal_alpha = 0.0            # alpha=0 → floor=0 → no clamping ever
        )
    )
    conv = NoLimits.get_diagnostics(res).convergence
...
@testset "SA anneal: closed-form variance floored in returned params" begin
    ...
    @test all(collect(θ.omega) .>= floor_final - 1.0e-8)
