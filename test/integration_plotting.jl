...
const _PLT_DF_P = DataFrame(
    ID = [1, 1, 2, 2], t = [0.0, 1.0, 0.0, 1.0], y = [0.1, 0.2, 0.0, -0.1]
)
...
const _PLT_RE_RES_LAP = fit_model(
    _PLT_RE_DM,
...
@testset "plot_fits MCMC" begin
    ...
    p = plot_fits(
        _PLT_RES_MCMC; mcmc_draws = 5, plot_mcmc_quantiles = true,
...
@testset "plot_fits VI" begin
    p = plot_fits(
        _PLT_RES_VI; mcmc_draws = 5, plot_mcmc_quantiles = true,
        mcmc_quantiles = [5, 95], mcmc_quantiles_alpha = 0.8
    )
    @test p !== nothing
end
...
@testset "plot_vpc validation errors" begin
    ...
    @test_throws ErrorException plot_vpc(
        _PLOT_RES_MLE;
...
@testset "plot_observation_distributions basic" begin
    p = plot_observation_distributions(
        _OBS_RES_MLE;
        individuals_idx = 1, obs_rows = 1, observables = :y
    )
    @test p !== nothing

    p_dm = plot_observation_distributions(
        _OBS_DM;
        individuals_idx = 1, obs_rows = 1, observables = :y
    )
    @test p_dm !== nothing
...
@testset "plot_observation_distributions caching" begin
    ...
    p_cache = plot_observation_distributions(
        _OBS_RES_MLE;
        individuals_idx = 1, obs_rows = 1, observables = :y,
    ...
    p_no_cache = plot_observation_distributions(
        _OBS_RES_MLE;
        individuals_idx = 1, obs_rows = 1, observables = :y
    )
