...
@testset "ODE solve integration" begin
    ...
    f! = get_de_f!(de)
    p = (;
        fixed_effects = ComponentArray(a = 1.0),
    ...
    sol = OrdinaryDiffEq.solve(prob, OrdinaryDiffEq.Tsit5(); abstol = 1.0e-9, reltol = 1.0e-9)
    @test isapprox(sol.u[end][1], exp(-1.0); rtol = 1.0e-5, atol = 1.0e-6)
end
...
@testset "ODE solve with helpers, random effects, preDE, and accessors" begin
    ...
    t = 0.3
    @test isapprox(accessors.x1(t), sol(t; idxs = 1); rtol = 1.0e-6, atol = 1.0e-8)
    @test isapprox(accessors.x2(t), sol(t; idxs = 2); rtol = 1.0e-6, atol = 1.0e-8)
    @test isapprox(
        accessors.s(t), helper_functions.sat(sol(t; idxs = 1)) + pre.pre;
