...
function _laplace_grad_matches_fd(dm; rtol, atol)
    ...
    inner = NoLimits.LaplaceInnerOptions(
        ...
        (maxiters = 50,), Optimization.AutoForwardDiff(), 1.0e-6
    )
    hessian = NoLimits.LaplaceHessianOptions(1.0e-6, 6, 10.0, false, 0.0, true, false, 0)
    cache_opts = NoLimits.LaplaceCacheOptions(0.0)
    multistart = NoLimits.LaplaceMultistartOptions(0, 0, 1.0e-6, 5, :lhs)
    obj_of = θ_vec -> begin
    ...
    return @test isapprox(grad_of(collect(θ0)), fd; rtol = rtol, atol = atol)
end
...
@testset "Laplace objective gradient matches FD (scalar RE)" begin
    _laplace_grad_matches_fd(fx_re_dm(); rtol = 1.0e-3, atol = 1.0e-3)
end
@testset "Laplace objective gradient matches FD (multivariate + multiple groups)" begin
    _laplace_grad_matches_fd(fx_mvn_dm(); rtol = 2.0e-3, atol = 2.0e-3)
end
@testset "Laplace objective gradient matches FD (ODE)" begin
    _laplace_grad_matches_fd(fx_ode_dm(); rtol = 2.0e-3, atol = 2.0e-3)
end
@testset "Laplace objective gradient matches FD (multiple RE groups)" begin
    _laplace_grad_matches_fd(fx_mg_dm(); rtol = 2.0e-3, atol = 2.0e-3)
end
...
function _outer_grad_matches_fd_t(dm, method, hmode; rtol, h = 3.0e-4)
    ...
    kw = (;
        inner = inner, hessian = method.hessian, cache_opts = method.cache,
    ...
    θu_of = θt -> layout.inv_transform(
        NoLimits._merge_free_into_full(
            layout.θ_const_t_vec, layout.free_idx,
            ComponentArray(collect(θt), layout.axs), layout.axs_full
        )
    )
    obj = θt -> NoLimits._laplace_objective_only(
        dm, binfos, θu_of(θt), const_cache,
        llc, fresh(); rng = Random.Xoshiro(0), kw...
    )

    θt0 = collect(layout.θ0_free_t)
    θt_full = NoLimits._merge_free_into_full(
        layout.θ_const_t_vec, layout.free_idx,
        layout.θ0_free_t, layout.axs_full
    )
    _, g_u, _ = NoLimits._laplace_objective_and_grad(
        dm, binfos, θu_of(θt0), const_cache,
        llc, fresh(); rng = Random.Xoshiro(0), kw...
    )
    ...
    return @test isapprox(collect(g_free), fd; rtol = rtol)
end
...
@testset "outer gradient matches FD at the defaults (nonlinear in η)" begin
    ...
    df = DataFrame(
        ID = repeat(1:5, inner = 4), t = repeat(0.0:3.0, 5),
        y = [
            1.35, 1.1, 0.92, 0.81, 1.62, 1.28, 1.05, 0.9,
            1.18, 0.99, 0.85, 0.74, 1.9, 1.47, 1.19, 1.02,
            1.05, 0.88, 0.77, 0.68,
        ]
    )
    model = @Model begin
    ...
    _outer_grad_matches_fd_t(dm, NoLimits.Laplace(), NoLimits._ExactHess(); rtol = 2.0e-3)
    _outer_grad_matches_fd_t(
        dm, NoLimits.FOCEI(), NoLimits._FOCEIHess(true); rtol = 2.0e-3
    )
    _outer_grad_matches_fd_t(
        dm, NoLimits.FOCEI(interaction = false),
        NoLimits._FOCEIHess(false); rtol = 2.0e-3
    )
end
...
@testset "Laplace uses level-specific constant covariates in RE priors" begin
    ...
    df = DataFrame(
        ID = [1, 1, 2, 2], SITE = [:A, :A, :B, :B], t = [0.0, 1.0, 0.0, 1.0],
    ...
    @test isapprox(
        ...
        ll + prior_sum; atol = 1.0e-8, rtol = 1.0e-8
    )
end
...
@testset "Laplace with constants_re fixes all REs for one individual" begin
    ...
    df = DataFrame(
        ID = [:id1, :id1, :id2, :id2], SITE = [:A, :A, :B, :B],
